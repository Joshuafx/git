...
def Xi(begin,end):
    ...
    for n in list_business_set:
        SQL = 'select ifnull(count(business),0) from open_event_data where business=\"' + n + '\"' +' and happentime between "'+str(begin)+'" and "'+str(end)+'"'
        cur.execute(SQL)
...
def Yi(Xievent):
    Yi_event = {}
    for n in list_business_set:
        Yi_event[n]=(compare_1(Ui_event_av[n],Xievent,n))
    return Yi_event