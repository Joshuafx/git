...
def XiE(begin, end):
    ...
    for n in list_eventname_set:
        SQL = 'select ifnull(count(eventname),0) from open_event_data where eventname=\"' + n + '\"' + ' and happentime between "' + str(begin) + '" and "' + str(end) + '"'
        cur.execute(SQL)
...
for n, m in zip(list_eventname_set, range(0, len_eventname)):
    SQL = 'select ifnull(count(eventname),0) from open_event_data where eventname=\"' + n + '\"' + ' and period between ' + \
...
def compare_1(arg1, X, n):
    if arg1 >= 1:
        return X[n] / Ui_event_av[n]
    elif arg1 < 1:
        return X[n] / SUMi_event[n]
...
def YiE(Xievent):
    ...
    for n in range(0, len_eventname):
        Yi_event.append(compare_1(Ui_event_av[n],Xievent,n))
    return Yi_event
